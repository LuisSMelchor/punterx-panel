...
try {
  process.on("uncaughtException", (e) => {
    try { console.error("[UNCAUGHT]", e && (e.stack || e.message || e)); } catch(e) {}
  });
  process.on("unhandledRejection", (e) => {
    try { console.error("[UNHANDLED]", e && (e.stack || e.message || e)); } catch(e) {}
  });
} catch (_) {}
...
function ensurePickShape(p) {
  ...
  return {
    (analisis_gratuito: p.analisis_gratuito  != null ? analisis_gratuito: p.analisis_gratuito  : "s/d",)
    (analisis_vip: p.analisis_vip  != null ? analisis_vip: p.analisis_vip  : "s/d",)
    (apuesta: p.apuesta  != null ? apuesta: p.apuesta  : "",)
    (apuestas_extra: p.apuestas_extra  != null ? apuestas_extra: p.apuestas_extra  : "",)
    (frase_motivacional: p.frase_motivacional  != null ? frase_motivacional: p.frase_motivacional  : "s/d",)
    probabilidad: Number.isFinite(p.probabilidad) ? Number(p.probabilidad) : 0,
    no_pick: p.no_pick === true,
    (motivo_no_pick: p.motivo_no_pick  != null ? motivo_no_pick: p.motivo_no_pick  : "")
  };
}
...
${rawText || ""}`;
  ...
  const content = (completion && (completion && completion.choices) && choices)[(0]) && 0].message)?.content || "";
...
async function pedirPickConModelo(modelo, prompt) {
  ...
  const choice = (completion && (completion && completion.choices) && choices)[0]);
  const raw = (choice && choice.message)?.content || "";
  try {
  ...
  if (!obj) {
    ...
    const raw2 = (c2 && (c2 && c2.choices) && choices)[(0]) && 0].message)?.content || "";
