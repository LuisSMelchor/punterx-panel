  ...
  try {
    ...
    if (qs.ping === '1') {
      return {
        ...
        body: JSON.stringify({ ok: true, ping: "scheduler_bridge_v2 (pong)" })
      };
    }

    // Proxy a run2 (simula cron)
    const base =
      (process.env && (process.env.URL || process.env.DEPLOY_PRIME_URL)) ||
    ...
    let resumen = null, raw_snippet = undefined;

    try {
      const j = JSON.parse(r.text || '');
      resumen = j && j.resumen ? j.resumen : null;
    } catch {
      raw_snippet = (r.text || '').slice(0, 4000);
    }
