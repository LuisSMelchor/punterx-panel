...
exports.handler = async (event) => {
  try {
    ...
    if (qs.ping === '1') {
      return { statusCode: 200, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ok:true, ping:'scheduler_bridge_v3 (pong)' }) };
    }
    ...
    let resumen = null, raw_snippet;
    try { const j = JSON.parse(r.text || ''); resumen = j && j.resumen ? j.resumen : null; }
    catch { raw_snippet = (r.text || '').slice(0, 4000); }

    return {
      statusCode: 200, headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ok:true, triggered:'run2', status:r.status, resumen, raw_snippet })
    };
  } catch (e) {
    return { statusCode: 500, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ok:false, error: (e && e.message) || String(e) }) };
  }
};
