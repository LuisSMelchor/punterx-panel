...
exports.handler = async (event = {}) => {
  try {
    ...
    // FAST PING (evita cualquier require/IO)
    if (qs.ping === '1') {
      return {
        statusCode: 200,
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ ok: true, ping: 'scheduler_bridge_v3 (pong)' })
      };
    }

    // Carga perezosa solo si vamos a proxyear
    ...
    const base = (process.env && (process.env.URL || process.env.DEPLOY_PRIME_URL))
