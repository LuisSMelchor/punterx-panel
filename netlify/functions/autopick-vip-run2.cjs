...
exports.handler = async (event, context) => {
  try {
    ...
    // 2) Carga diferida del impl, evitando bundling agresivo
    let impl;
    try {
      const rq = eval('require');               // <- importante para cold start
    ...
    // Inyecta auth solo para cron o ejecución manual
    ...
    // 5) Validar respuesta y forzar JSON si hace falta
