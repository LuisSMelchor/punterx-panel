// netlify/functions/_diag-core-v3.cjs
// Núcleo compartido del diagnóstico (checks + render + actividad de picks)
// UI pro + Montreal por defecto + build tag visible (v3)
...
async function fetchPicksSnapshot({ limit = 30, hours = 24 } = {}) {
  ...
  try {
    ...
    // 1) intento con created_at; 2) fallback sin created_at
...
module.exports = { runChecks, runDeepActivity, renderHTML };
